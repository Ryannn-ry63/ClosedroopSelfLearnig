...
class data_collection:
    ...
    def __init__(self):
        ...
        self.control_info_record = pd.DataFrame(columns=('u', 'du', 'Cur_acc', 'q', 'ru', 'rdu'))
    ...
    def data_record(self, info, name):
        if name == "control_info":
            self.control_info_record = self.control_info_record.append(
                pd.DataFrame(
                    {info[0]: [info[1]], info[2]: [info[3]], info[4]: [info[5]], info[6]: [info[7]], info[8]: [info[9]],
                     info[10]: [info[11]]}),
                ignore_index=True)
            self.control_info_record.to_csv(self.directory_path + '/' + self.file_name_1,
                                            index=False)
            ...
            self.obs_info_record = self.obs_info_record.append(
                pd.DataFrame({info[0]: [info[1]], info[2]: [info[3]], info[4]: [info[5]], info[6]: [info[7]]}),
                ignore_index=True)
            self.obs_info_record.to_csv(self.directory_path + '/' + self.file_name_3,
                                        index=False)
