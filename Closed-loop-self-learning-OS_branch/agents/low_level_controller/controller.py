...
class VehiclePIDController:
    ...
    def run_step(self, target_speed, waypoint):
        ...
        """
        if type(target_speed) ==np.ndarray:
...
class Lanechangemodel:
    ...
    def run_step(self, targetwp, vehicle_ahead):
        if vehicle_ahead is None:
            nextwp=targetwp
        else:
            loc1 = [vehicle_ahead.get_location().x, vehicle_ahead.get_location().y, vehicle_ahead.get_location().z]
            loc2 = [self.vehicle.get_location().x, self.vehicle.get_location().y, self.vehicle.get_location().z]
            loc1b = self.inertial_to_body_frame(xi=loc1[0], yi=loc1[1])
            loc2b = self.inertial_to_body_frame(xi=loc2[0], yi=loc2[1])
            d = - loc2b[0] + loc1b[0]
            s = loc2b[1] - loc1b[1]
            if d < 20 and s < 0.6*self.LANE_WIDTH:
                targetwpb = self.inertial_to_body_frame(xi=targetwp[0], yi=targetwp[1])
                nextwp = self.body_to_inertial_frame(xb=targetwpb[0], yb=targetwpb[1] + self.LANE_WIDTH)
            else:
                nextwp=targetwp
